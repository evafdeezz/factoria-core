...
public class MenstrualCycleController {
    ...
    public static class CycleStatusDto {
        public int cycleDay;
        public int daysOverdue;        // > 0 si se ha pasado del ciclo esperado
        public boolean isLate;         // true si daysSince >= cycleLength
        public String phase;           // null si isLate
    ...
    public ResponseEntity<?> getCycleStatus(@PathVariable Long athleteId) {
        ...
        if (daysSince >= cycleLength) {
            ...
            // Ciclo en curso normalmente
            dto.cycleDay    = cycleDay;
            dto.daysOverdue = 0;
            dto.isLate      = false;

            MenstrualPhase phase = calculatePhase(cycleDay, menstrualDuration, cycleLength);
            buildPhaseInfo(dto, phase);
        }
