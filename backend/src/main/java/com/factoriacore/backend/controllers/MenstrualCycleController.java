...
public class MenstrualCycleController {
    ...
    private void checkAccess(AppUserDetails currentUser, AthleteProfile profile) {
        ...
        boolean isCoachWithAccess = currentUser.getUser().getRole() == UserRole.COACH
                ...
                profile.getId(), currentUser.getUser().getId());
    ...
    public static class CycleStatusDto {
        public Long cycleId;        // ← nuevo: id del ciclo activo
        public int cycleDay;
    ...
    public ResponseEntity<?> getCycleStatus(@PathVariable Long athleteId,
        ...
        // Rellenar cycleId con el ciclo más reciente de la atleta
        ...
        if (daysSince >= cycleLength) {
            ...
            dto.training    = "En esta fase de incertidumbre, escucha a tu cuerpo. Si notas los síntomas previos habituales (molestias, retención, irritabilidad), ajusta la carga como en fase lútea tardía: prioriza técnica, movilidad y evita los tests de máxima intensidad.";
    ...
    public ResponseEntity<?> registerPeriod(@PathVariable Long athleteId,
        ...
        if (!profile.getUser().getId().equals(currentUser.getUser().getId()))
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "ACCESS_DENIED");
    ...
    private void buildPhaseInfo(CycleStatusDto dto, MenstrualPhase phase) {
        ...
        switch (phase) {
            case MENSTRUAL -> {
                dto.phaseLabel = "Fase menstrual"; dto.emoji = "🔴";
            ...
            case FOLLICULAR -> {
                dto.phaseLabel = "Fase folicular"; dto.emoji = "🟡";
            ...
            case OVULATORY -> {
                dto.phaseLabel = "Fase ovulatoria"; dto.emoji = "🟢";
            ...
            case LUTEAL -> {
                dto.phaseLabel = "Fase lútea"; dto.emoji = "🔵";
