...
export default function MenstrualSettingsPage() {
  ...
  const validateForm = () => {
    ...
    if (!isValidPeriodDate(form.lastPeriodDate)) {
    ...
    if (form.menstrualDuration < 2 || form.menstrualDuration > 10) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await saveAthleteProfile(user.id, {
        ...
        shareMenstrualDataWithCoach: form.menstrualEnabled
          ? form.shareWithCoach
          : false,
        cycleLength: form.menstrualEnabled ? form.cycleLength : null,
        menstrualDuration: form.menstrualEnabled
          ? form.menstrualDuration
          : null,
        lastPeriodDate: form.menstrualEnabled ? form.lastPeriodDate : null,
      });
      ...
      <div className="max-w-xl mx-auto space-y-5">
        ...
        <header className="space-y-1">
          <p className="text-[10px] tracking-[0.2em] uppercase text-sky-300">
            Ajustes · Ciclo menstrual
          </p>
          <h1 className="text-2xl font-semibold">Ciclo menstrual</h1>
          <p className="text-xs text-slate-400">
            Registrar tu ciclo ayuda a adaptar la planificación a tu cuerpo. Tú
            controlas qué datos se guardan y quién puede verlos.
          </p>
        </header>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 space-y-5">
            <label className="flex items-center justify-between cursor-pointer select-none">
              ...
              <div
                onClick={() =>
                  setForm((f) =>
                    f ? { ...f, menstrualEnabled: !f.menstrualEnabled } : f
                  )
                }
                className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer flex-shrink-0 ml-4 ${
                  form.menstrualEnabled ? "bg-sky-500" : "bg-slate-600"
                }`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all ${
                    form.menstrualEnabled ? "left-6" : "left-1"
                  }`}
                />
              </div>
            </label>
              ...
              <>
                <div className="border-t border-slate-700 pt-5 space-y-4">
                        ...
                        onChange={(e) =>
                          setForm((f) =>
                            f
                              ? {
                                  ...f,
                                  menstrualDuration: Number(e.target.value),
                                }
                              : f
                          )
                        }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((f) =>
                          f ? { ...f, lastPeriodDate: e.target.value } : f
                        )
                      }
                ...
                <div className="border-t border-slate-700 pt-5">
                  <label className="flex items-center justify-between cursor-pointer select-none gap-4">
                    <div>
                      <p className="text-sm font-medium text-slate-200">
                        Compartir con mi entrenadora
                      </p>
                      <p className="text-xs text-slate-400 mt-0.5">
                        Tu entrenadora podrá ver la fase del ciclo al planificar.
                    ...
                    <div
                      onClick={() =>
                        setForm((f) =>
                          f ? { ...f, shareWithCoach: !f.shareWithCoach } : f
                        )
                      }
                      className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer flex-shrink-0 ${
                        form.shareWithCoach ? "bg-sky-500" : "bg-slate-600"
                      }`}
                    >
                      <div
                        className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all ${
                          form.shareWithCoach ? "left-6" : "left-1"
                        }`}
                      />
                    </div>
                  </label>
