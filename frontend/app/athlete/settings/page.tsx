...
export default function AthleteSettingsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateUser(user.id, { fullName: form.fullName, email: form.email });
  ...
  if (user.role !== "ATHLETE") return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-sky-900 text-slate-100">
      <p className="text-xs text-slate-300">Esta pantalla es solo para atletas.</p>
    </div>
  );
      ...
      <div className="max-w-xl mx-auto space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 space-y-4">
            <p className="text-xs font-semibold text-sky-300 uppercase tracking-wider">Datos personales</p>
            ...
            <div>
              <label className="block text-[11px] font-medium text-slate-300 mb-1">Fecha de nacimiento</label>
              {/* [color-scheme:dark] makes the calendar icon white */}
            ...
            <div>
              <label className="block text-[11px] font-medium text-slate-300 mb-2">Sexo</label>
              <div className="grid grid-cols-3 gap-2">
                {([
                  ...
                  <button key={opt.value} type="button"
                    onClick={() => setForm((f) => f ? { ...f, sex: opt.value } : f)}
          ...
          {/* Enlace a ajustes del ciclo menstrual — solo si es mujer */}
          {isFemale && (
            <button type="button" onClick={() => router.push("/athlete/settings/menstrual")}
              ...
              <div className="text-left">
                <p className="text-sm font-medium text-slate-200">Ciclo menstrual</p>
                <p className="text-xs text-slate-400 mt-0.5">
                  Configura el seguimiento de tu ciclo y tus preferencias de privacidad.
                </p>
              </div>
